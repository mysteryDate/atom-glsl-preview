...
module.exports = {
  ...
  // Auto-format shadertoy code
  // TODO:
  //   - Handle "in" and "out" in main (become gl_FragCoord and gl_FragColor, respectively)
  //   - Textures, simply put them at the top with a dummy REPLACE_ME path
  //   - All function calls to texture() should be texture2D()
  //   - iDate, not sure what to do here
  importFromShaderToy() {
    ...
    textBuffer.replace(/void\s*main(Image)?\s*\(.*\)/, "void main()");
    textBuffer.replace(/fragCoord[^.]/g, "fragCoord.xy"); // Don't assume fragCoord is a vec2
    Object.keys(textReplacements).forEach((word) => {
      const reg = new RegExp(word, "g");
      textBuffer.replace(reg, textReplacements[word]); // replace all instances of word
    });
  },
